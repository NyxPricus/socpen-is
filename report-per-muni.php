...
<table class = "table table-bordered table-condensed">																			
    ...
    <?php
 // $rank = 0;
$countquery2 = "SELECT ul.city_muni,sum(case when ulop.year = 2018 then 1 else 0 end) as y2018,sum(case when ulop.year = 2019 then 1 else 0 end) as y2019, sum((case when ulop.year = 2018 then 1 else 0 end) + (case when ulop.year = 2019 then 1 else 0 end)) as total
FROM `uct_listahanan_paid` ulop
join uct_list_110 ul on ul.hhid = ulop.hhid
where ul.province = '" . $province_get . "'
GROUP by ul.city_muni
...
while($rowStatus2 = $countresult2->fetch_assoc()){
    ...
    $muni = str_replace("&","AND",$rowStatus2['city_muni']) ;
    $totalcounter += $total;
   // $rank += 1 ;
    $total2018 += $y2018;
    $total2019 += $y2019;
...
<td><a href="report-per-barangay?province=<?php echo $province_get?>&muni=<?php echo $muni?>" target="_Blank"><?php echo $muni?></a></td>
